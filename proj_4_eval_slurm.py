...
pivot_row = 6000000
#pivot_row = 600
def get_neg_dataset(path="/bigtemp/rm5tx/nlp_project/2016-07_all.csv"):
    print("reading neg")
    data = pd.read_csv(path, dtype={'body':str},skiprows=range(1,pivot_row*device_id),nrows=pivot_row)
...
def main():
    out_path="/bigtemp/rm5tx/nlp_project/2016-07_all_predicted_adj_model_"+str(device_id)+".csv"
    #model = ProjModel.load_from_checkpoint(checkpoint_path=os.path.expanduser("/u/rm5tx/nlp_project/final_base_model.ckpt"))
    model = ProjModel.load_from_checkpoint(checkpoint_path=os.path.expanduser("/u/rm5tx/nlp_project/final_adj_model.ckpt"))
    
    
    DATA_PATH = os.path.expanduser("~/data_cache/")
    
    logging.basicConfig(filename='log_inference_slurm'+str(device_id)+'.log', filemode='w', format='%(message)s', level=logging.INFO)
    mylogger = logging.getLogger('Admin_Client')
    mylogger.info('hello')
    
    data = ProjData(max_len=128, ratio=1, adjacent=True, adjrat=0.23, adjtot=2)
    data.load(DATA_PATH)
    ...
    device_string = "cuda:"+str(device_id%4)
    device = torch.device(device_string)
    masked_input = TensorDataset(inputs, masks)
    dataloader = DataLoader(masked_input, batch_size=64)
    model.eval()
