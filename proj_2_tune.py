...
def tunerun(config):
    ...
    data = ProjData(max_len=MAX_LEN, ratio=RATIO, adjacent=ADJACENT, adjrat=DAT_RAT, adjtot=DAT_TOT)
    if ADJACENT:
    ...
    trainer = pl.Trainer(progress_bar_refresh_rate=100, # logger=logger,
...
reporter = CLIReporter(
    parameter_columns=["max_len", "ratio", "lr", "thresholds"],
    metric_columns=["loss", "avg_loss", "avg_acc"])



config = {
    # "thresholds" : (None, None), 
    # "thresholds" : tune.grid_search([(1.0, 0), (0.5, 0)]),
    # "thresholds" : tune.grid_search([(0.3, 2), (0.2, 5)]),
    # "thresholds" : tune.grid_search([(.16, 5), (.23, 2), (.33, 0), (.5, 0)]),
    "thresholds" : tune.grid_search([(.16, 5), (.23, 2), (.33, 0), (.5, 0)]),
    # "ratio" : tune.grid_search([1, 2]),
    "ratio" : 0.5,
    "max_len": 128,
    "train_batch_size": 32,
    "val_batch_size": 32,
